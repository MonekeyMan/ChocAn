...
javac -d bin src/chocan/*.java 2>&1
...
javac -d bin/tests -cp "bin:$JUNIT_JAR:$HAMCREST_JAR" tests/chocan/*.java 2>&1
...
echo "Running tests..."
echo "-----------------------------------"
java -cp "bin:bin/tests:$JUNIT_JAR:$HAMCREST_JAR" org.junit.runner.JUnitCore chocan.AllTests
